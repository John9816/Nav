...
const getHeaders = () => {
  return {
    'X-API-Key': API_KEY,
    // 'Content-Type': 'application/json' // Not strictly needed for GET, but good practice if POST
  };
};
...
const safeJson = async (response: Response) => {
    ...
    // If text/html or other, treat as error or handle empty
    console.warn(`Received non-JSON response from ${response.url}:`, contentType);
    return null;
};
...
export const fetchSongDetail = async (id: number | string): Promise<Song | null> => {
  try {
    // Use aggregator to fetch detail via method dispatch
