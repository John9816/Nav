...
const BookmarkBar: React.FC<BookmarkBarProps> = ({ categories }) => {
  ...
  return (
    <div className="max-w-[1920px] mx-auto px-4 sm:px-6 min-h-[52px] flex items-center py-2">
      <div className="flex items-center gap-2 w-full h-full overflow-x-auto flex-nowrap md:flex-wrap scrollbar-hide md:overflow-visible">
        <div className="flex items-center gap-2 px-3 py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 select-none shrink-0 mr-2 rounded-full bg-white/55 dark:bg-slate-900/55 border border-white/60 dark:border-slate-700/60">
        ...
        {rootNode.links.map((link) => (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-3 py-2 rounded-full text-xs font-medium text-slate-700 dark:text-slate-200 hover:bg-white/70 dark:hover:bg-slate-800/70 hover:text-orange-600 dark:hover:text-teal-300 transition-colors whitespace-nowrap shrink-0 max-w-[220px]"
