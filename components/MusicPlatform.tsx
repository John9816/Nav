...
import {
  ...
  ChevronDown, Repeat1, ArrowDown, Menu, Sparkles, Calendar, ListMusic, PlayCircle, Trash2, Globe, Server, Info, Share2
} from 'lucide-react';
...
const MusicPlatform: React.FC<MusicPlatformProps> = ({
  ...
  // Netease playlist categories
  const [playlistCategories, setPlaylistCategories] = useState<Array<{ name: string; sub?: Array<{ name: string }> }>>([]);
  ...
  useEffect(() => {
    ...
    // Load Netease playlist categories (for playlists page)
    fetchPlaylistCategories().then(setPlaylistCategories).catch(e => console.warn('Failed to load playlist categories', e));
  }, []);
  ...
  const loadPlaylists = async (opts?: { reset?: boolean; cat?: string }) => {
      const reset = opts?.reset ?? false;
      const cat = opts?.cat ?? selectedCat;

      if (isLoadingPlaylists) return;

      setIsLoadingPlaylists(true);
      try {
          const nextOffset = reset ? 0 : playlistOffset;
          const data = await fetchPlaylists(42, nextOffset, cat);
          ...
          setHasMorePlaylists(got >= 42);
          setPlaylistOffset(nextOffset + got);
      } catch (e) {
  ...
  useEffect(() => {
      if (view === 'playlists' && playlists.length === 0 && !isLoadingPlaylists) {
          ...
          setPlaylistOffset(0);
          loadPlaylists({ reset: true, cat: selectedCat });
      }
  }, [view]);
                           ...
                           <>
                               {(view === 'history' || view === 'search' || view === 'favorites' || view === 'playlist') && (
                                   <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
                                       ...
                                       {/* List Header - Sticky */}
                                       <div className="sticky top-[72px] z-10 grid grid-cols-[50px_1fr_72px] md:grid-cols-[60px_4fr_3fr_120px] gap-4 px-6 md:px-10 py-3 bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur border-b border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-400 uppercase tracking-wider shadow-sm">
                                       ...
                                       <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
                                           {(view === 'search' ? searchResults : playlistSongs).map((song, i) => (
                                               <div
                                                 ...
                                                 className={`group grid grid-cols-[50px_1fr_72px] md:grid-cols-[60px_4fr_3fr_120px] gap-4 px-6 md:px-10 py-3.5 items-center transition-all cursor-pointer
                                               ...
                                               >
                                                   ...
                                                   <div className="text-right md:hidden flex justify-end gap-1">
                                                       <button
                                                         onClick={(e) => handleShareSong(e, song)}
                                                         className="p-2 text-slate-400 hover:text-blue-500 transition-colors"
                                                         title="复制分享链接"
                                   ...
                                   <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
                                       <div className="px-6 md:px-10 py-8">
                                            ...
                                            <div className="mb-6 flex flex-col gap-3">
                                                <div className="flex flex-wrap gap-2">
                                                    <button
                                                        ...
                                                        className={`px-3 py-1.5 rounded-full text-xs font-bold border transition-colors ${selectedCat === '' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}
                                                    >
                                                        全部
                                                    </button>
                                                    {playlistCategories.flatMap(g => (g.sub || [])).slice(0, 24).map((c) => (
                                                        <button
                                                            key={c.name}
                                                            onClick={() => {
                                                                closeLyrics();
                                                                ...
                                                                setPlaylistOffset(0);
                                                                setHasMorePlaylists(true);
                                                                loadPlaylists({ reset: true, cat: c.name });
                                                            }}
                                                            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${selectedCat === c.name ? 'bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-900/50' : 'bg-white/60 dark:bg-slate-800/40 text-slate-600 dark:text-slate-300 border-slate-200/60 dark:border-slate-700/60 hover:bg-slate-50 dark:hover:bg-slate-700/50'}`}
                                                        >
                                                            {c.name}
                                                        </button>
                                                    ))}
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-6">
                                                ...
                                                {playlists.length === 0 && (
                                                    <div className="col-span-full text-center py-20 text-slate-400">
                                                        {isLoadingPlaylists ? '歌单加载中...' : '暂无歌单数据'}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            {playlists.length > 0 && hasMorePlaylists && !autoLoadPlaylists && (
                                                <div className="pt-10 flex justify-center">
                                                    <button
                                                        ...
                                                        className="flex items-center gap-2 px-8 py-3 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors disabled:opacity-50"
                                                    >
                                                        {isLoadingPlaylists ? <Loader2 size={16} className="animate-spin" /> : <ArrowDown size={16} />}
                                                        {isLoadingPlaylists ? '正在加载...' : '加载更多歌单'}
                                                    </button>
                                                </div>
                                            )}
                                       </div>
